...
<BODY  onload="f_frameStyleResize(self,10)" style="overflow-x:auto;overflow-y:hidden" >
  ...
  <script language="javaScript">
      ...
      bar.setMessage('<@getMessage/>');
       bar.addItem("<@text name="action.info"/>",'info()');
      ...
      function info(){
         ...
         window.open("feeDetail.action?method=info&feeDetail.id="+id, '', 'scrollbars=yes,left=0,top=0,width=600,height=350,status=yes');
      }

      function exportData() {
        form.action = "feeSearch.action?method=export";
        var propertyKeys = "createAt,std.code,std.name,semester.schoolYear,semester.name,type.name,mode.name,shouldPay,payed,invoiceCode,toRMB,currencyCategory.name,rate,remark,HSKGrade,std.grade";
          var propertyShowKeys = "缴费日期,学号,姓名,学年度,学期,收费类型,缴费方式,应缴金额,实交金额,发票号,折合人民币,币种,汇率,备注,HSK成绩,年级";
