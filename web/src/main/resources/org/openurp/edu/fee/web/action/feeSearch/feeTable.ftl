   <@table.table align="center" sortable="true" id="feeDetail" width="1400px">
     <@table.thead style="font-size: 12px;height:10px">
       ...
       <@table.sortTd name="field.feeDetail.date" id="feeDetail.createdAt"/>
       ...
       <@table.sortTd name="attr.term" id="feeDetail.semester.schoolYear"/>
       ...
       <@table.td text="HSK成绩"/>
       <@table.sortTd text="年级" id="feeDetail.std.grade"/>
     </@>
     <@table.tbody  style="font-size: 12px;height:10px" datas=fees;feeDetail>
       <@table.selectTd id="feeDetailId" value=feeDetail.id/>
      <td>${feeDetail.createdAt?if_exists?string("yyyy-MM-dd")}</td>
      <td><a href="#" onclick="javascript:location='studentSearch!info.action?studentId=${feeDetail.std.id}'">${feeDetail.std?if_exists.code}</a></td>
      <td>${feeDetail.std?if_exists.name?if_exists}</td>
      <td>${feeDetail.semester.schoolYear} ${feeDetail.semester.name}</td>
      ...
      <td>${feeDetail.std.grade?if_exists}</td>
     </@>
   </@>
  <br><br>
