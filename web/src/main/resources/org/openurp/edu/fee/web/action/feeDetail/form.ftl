...
[@b.head/]
  ...
  [@b.form name="feeDetailForm" action="!save" target="feeDetails" theme="list"]
    ...
    [#if (feeDetail.id)?exists]
      ...
      [@b.field label="收费部门"]<span style="display: inline-block;">${feeDetail.depart.name}</span>[/@]
      [@b.field label="交费类型"]<span style="display: inline-block;">${feeDetail.type.name}</span>[/@]
      [@b.field label="应缴费用"]<span style="display: inline-block;">${feeDetail.shouldPay?string("0.00#")}</span>[/@]
      [@b.textfield label="本次缴费" name="feeDetail.payed" value=(feeDetail.payed?string("0.00#"))! required="true" maxlength="64" check="match('number')" style=elementSTYLE/]
    [#else]
      ...
      [@b.textfield label="应缴费用" name="feeDetail.shouldPay" value=(feeDetail.shouldPay?string("0.00#"))! required="true" maxlength="64" check="match('number')" style=elementSTYLE/]
    ...
    [#if (feeDetail.id)?exists]
      [@b.datepicker label="实缴日期" name="feeDetail.payedAt" value=(feeDetail.payedAt)! required="true" style=elementSTYLE comment="（此处为本次实际缴费日期，即最终最后一次缴费时间）"/]
    [#else]
      [@b.datepicker label="实缴日期" name="feeDetail.payedAt" value=(feeDetail.payedAt)! style=elementSTYLE comment="（此处为本次实际缴费日期，即最终最后一次缴费时间）"/]
    [/#if]
